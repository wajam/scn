...
class TestZookeeperTimestampStorage extends FunSuite {
  ...
  test("counter head position in zookeeper") {
    ...
    assert(storage.head.compareTo(Timestamp(System.currentTimeMillis() + storage.SAVE_AHEAD_MS)) == -1,
      storage.head.compareTo(Timestamp(System.currentTimeMillis() + storage.SAVE_AHEAD_MS)))
  }
